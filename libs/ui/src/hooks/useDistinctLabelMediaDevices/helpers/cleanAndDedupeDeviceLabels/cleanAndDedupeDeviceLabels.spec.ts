...
import cleanAndDedupeDeviceLabels from '.';
...
const createMockAudioDevice = (deviceId: string, label: string): MediaDeviceInfoJSON => ({
  deviceId,
  label,
  kind: 'audioinput',
  groupId: 'default',
...
describe('cleanAndDedupeDeviceLabels', () => {
  ...
  describe('Edge Cases', () => {
    it.each([
      ...
      ['empty array', [] as MediaDeviceInfoJSON[], []],
  ...
  describe('Device Type Support', () => {
    it('works with mixed device types', () => {
      const devices: MediaDeviceInfoJSON[] = [
        createMockDevice('1', 'Camera (0000:0001)'),
        { deviceId: '2', label: 'Speaker (0000:0002)', kind: 'audiooutput', groupId: 'default' },
      ];

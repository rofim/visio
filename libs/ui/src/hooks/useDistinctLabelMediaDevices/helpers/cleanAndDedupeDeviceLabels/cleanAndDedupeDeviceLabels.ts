 ...
 * @returns {MediaDeviceInfoJSON[]} The list of devices with cleaned and deduplicated labels.
 */
const cleanAndDedupeDeviceLabels = (devices: MediaDeviceInfoJSON[]): MediaDeviceInfoJSON[] => {
  const labelCounts = new Map<string, number>();

  return devices.map((device): MediaDeviceInfoJSON => {
    if (!device.label) {
      return device;
    }

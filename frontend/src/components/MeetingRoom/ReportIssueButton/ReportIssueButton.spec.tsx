...
describe('ReportIssueButton', () => {
  it('should have a white icon when the form is closed', () => {
    ...
    expect(screen.getByTestId('vivid-icon-feedback-solid')).toHaveClass('text-vera-on-secondary');
  });
  it('should have a blue icon when the form is open', () => {
    render(<ReportIssueButton handleClick={() => {}} isOpen />);
    expect(screen.getByTestId('vivid-icon-feedback-solid')).toHaveClass('text-vera-secondary');
  });
